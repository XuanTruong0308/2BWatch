...
public interface WatchRepository extends JpaRepository<Watch, Integer> {
    ...
    // // Top 3 bán chạy nhất
    // @Query("SELECT w FROM Watch w WHERE w.isActive = true ORDER BY w.soldCount
    // DESC")
    // List<Watch> findTop3BestSellers();

    // // Top 3 mới nhất
    // @Query("SELECT w FROM Watch w WHERE w.isActive = true ORDER BY w.createdDate
    // DESC")
    // List<Watch> findTop3Newest();

    // // Top 3 giảm giá sâu nhất
    // @Query("SELECT w FROM Watch w WHERE w.isActive = true AND w.discountPercent >
    // 0 ORDER BY w.discountPercent DESC")
